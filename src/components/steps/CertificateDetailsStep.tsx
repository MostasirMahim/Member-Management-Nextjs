...
export default function CertificateDetailsStep() {
  ...
  const { mutate: deleteCertificateFunc, isPending: isDeleting } = useMutation({
    mutationFn: async (id: any) => {
      const res = await axiosInstance.delete(
        `/api/member/v1/members/certificate/${id}/`,
    ...
    onSuccess: async (data) => {
      if (data?.status === "success") {
        ...
        toast.success(data.message || "Certificate successfully deleted.");
      }
    },
    onError: (error: any) => {
